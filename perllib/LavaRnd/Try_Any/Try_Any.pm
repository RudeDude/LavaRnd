...
# This allows declaration	use LavaRnd::Try_Any ':all';
...
require XSLoader;
XSLoader::load('LavaRnd::Try_Any', $VERSION);
...
LavaRnd::Try_Any - use LavaRnd lavapool or any s100 quality if unavailable

=head1 SYNOPSIS

  use LavaRnd::Try_Any qw(
    random8 random16 random32 random_data
    random_val random_coin
    range_drandom range_dcrandom
    drandom dcrandom
  ...
  # Error checking
  #
  # NOTE: This LavaRnd::Try_Any module will exit instead of returning
...
If a fails to communicate with lavapool, then
a local s100 generator is tried.
Unlike LavaRnd::TryOnce_Any, this module will continue to
first try using the lavapool daemon before falling
back to the local s100 generator.

There is no quality restriction on values returned
by local s100 generator after a lavapool daemon failure.
While generally OK, the local s100 generator returns
non-cryptographically strong pseudo-random data.

This module uses the LavaRnd liblava_try_any.a library.
